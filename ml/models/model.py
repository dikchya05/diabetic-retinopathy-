...
def train_loop(labels_df, img_dir, model_name='efficientnet_b0', image_size=224,
               epochs=10, batch_size=16, lr=2e-4, device=None, out_dir='ml/models', num_workers=4):
    set_seed(42)
    if device is None:
    ...
    # Use 'label' column for stratification and labels
    ...
    train_ds = RetinopathyDataset(train_df, img_dir, transforms=train_t,
                                  image_column='id_code', label_column='label')
    val_ds = RetinopathyDataset(val_df, img_dir, transforms=valid_t,
                                image_column='id_code', label_column='label')
